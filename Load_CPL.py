...
x_DE = BundleSolution1D(nets, conditions)


def H_CPL(z, w_0, w_1, Om_m_0, H_0, x_DE):
    ...
    +\left(1-\Omega_{m,0}\right)\tilde{x}_{DE}}.`
    ...
    x_DEs = x_DE(z, w_0, w_1, to_numpy=True)

    H = H_0*((Om_m_0*((1+z)**3) + (1-Om_m_0)*x_DEs) ** (1/2))
...
for w_0 in np.linspace(-1.4, -0.6, 3):
    for w_1 in np.linspace(-5, 1.7, 2):
        Hs = H_CPL(zs, w_0, w_1, 0.3, 73, x_DE)
